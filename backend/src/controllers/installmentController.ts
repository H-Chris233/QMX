...
export class InstallmentController {
    ...
    try {
      ...
      if (firstInstallment) {
        const paymentRecord = await CashBuilder.create()
          ...
          .note(this.buildInstallmentNote(sanitizedNote, firstInstallment.current_installment, totalInstallmentsInt))
          .installment({
            plan_uid: plan.uid,
        ...
        const paidInstallment = await Installment.updateByUid(firstInstallment.uid, {
          status: InstallmentStatus.PAID,
  ...
  public updateInstallmentPayment = catchAsync(async (req: Request, res: Response): Promise<void> => {
    const { id } = req.params;
    const { status, amount } = req.body as { status: InstallmentStatus; amount?: number };
    ...
    try {
      if (normalizedStatus === InstallmentStatus.PAID) {
        const paidAmountInCents = amount !== undefined ? convertAmountToCents(amount) : installment.installment_amount;
        ...
        const cashRecord = await CashBuilder.create()
          ...
          .studentId(plan?.student_id ?? null)
          .note(this.buildInstallmentNote(plan?.note ?? null, installment.current_installment, installment.total_installments))
          .installment({
      ...
      const refreshedPlan = plan ? await InstallmentPlan.findByUid(plan.uid) : null;
      const responseData = {
        installment: this.presentInstallment(updatedInstallment),
        plan: refreshedPlan ? await this.buildPlanResponse(refreshedPlan) : null,
      };

      logger.info(`更新分期付款状态成功，ID: ${installment.uid}, 状态: ${normalizedStatus}`);
      res.json({
        success: true,
        data: responseData,
        message: '分期付款状态更新成功',
  ...
  public getOverdueInstallments = catchAsync(async (req: Request, res: Response): Promise<void> => {
    ...
    const enriched = await Promise.all(overdueInstallments.map(async installment => {
      const plan = await InstallmentPlan.findByUid(installment.plan_id);
      ...
      return {
        ...
        plan: plan ? {
          ...
          frequency_text: this.getFrequencyText(plan.frequency, plan.custom_days ?? undefined),
    ...
    const totalOverdueAmount = enriched.reduce((sum, item) => sum + (item.overdueAmountInCents ?? 0), 0);
    const totalDays = enriched.reduce((sum, item) => sum + (item.daysOverdue ?? 0), 0);
    ...
    logger.info(`获取逾期分期付款统计成功，逾期数量: ${count}, 逾期金额: ¥${this.formatAmount(totalOverdueAmount).toFixed(2)}`);
    res.json(response);
  });
  ...
  private async buildPlanResponse(plan: IInstallmentPlanDoc, options: IPlanResponseOptions = {}): Promise<Record<string, unknown>> {
    const installments = options.installments ?? await Installment.findByPlanId(plan.uid);
    const student = plan.student_id ? await Student.findByUid(plan.student_id) : null;
    ...
    if (options.includeInstallments) {
      baseResponse.installments = installments.map(inst => this.presentInstallment(inst));
    }
  ...
  private calculatePlanStats(installments: IInstallmentDoc[]) {
    return installments.reduce((acc, installment) => {
      ...
      if (status === InstallmentStatus.PAID) {
        acc.paidCount += 1;
        acc.paidAmount += installment.paid_amount ?? installment.installment_amount;
  ...
  private async refreshPlanStatus(planUid: number): Promise<void> {
    ...
    const allPaid = installments.every(inst => inst.status === InstallmentStatus.PAID);
    const activeExists = installments.some(inst => inst.status === InstallmentStatus.PENDING || inst.status === InstallmentStatus.OVERDUE);
  ...
  private normalizeInstallmentStatus(value: unknown): InstallmentStatus | null {
    ...
    const matched = Object.values(InstallmentStatus).find(item => item === value);
    return matched ?? null;
  }
