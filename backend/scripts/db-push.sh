...
run_drizzle_push() {
  ...
  if [ -n "$host_addr" ]; then
    output="$(DATABASE_URL_MIGRATION_HOSTADDR="$host_addr" NODE_OPTIONS=--dns-result-order=ipv4first drizzle-kit push "$@" 2>&1)"
  else
    output="$(NODE_OPTIONS=--dns-result-order=ipv4first drizzle-kit push "$@" 2>&1)"
  fi
  exit_code=$?
...
if [ -z "$db_url" ]; then
  echo '[db:push] 缺少 DATABASE_URL_MIGRATION 或 DATABASE_URL，无法执行回退' >&2
  exit 1
fi
...
if [ -z "$db_host" ]; then
  echo '[db:push] 回退失败：无法从数据库 URL 解析主机名' >&2
  exit 1
fi
...
if [ -z "$host_addr_list" ]; then
  echo "[db:push] 回退失败：无法解析 $db_host 的 IPv4 地址" >&2
  exit 1
fi

for host_addr in $host_addr_list; do
  echo "[db:push] fallback hostaddr=$host_addr (resolved from $db_host)"
  if run_drizzle_push "$host_addr" "$@"; then
    exit 0
  fi
done
